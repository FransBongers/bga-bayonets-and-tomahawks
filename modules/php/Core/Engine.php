...
use BayonetsAndTomahawks\Helpers\QueryBuilder;
use BayonetsAndTomahawks\Helpers\UserException;
...
class Engine
{
  ...
  {
    if (!self::$tree instanceof \BayonetsAndTomahawks\Core\Engine\SeqNode) {
      self::$tree = new \BayonetsAndTomahawks\Core\Engine\SeqNode([], [self::$tree]);
      self::save();
    }
  }
  ...
  public static function buildTree($t)
  {
    ...
    $className = '\BayonetsAndTomahawks\Core\Engine\\' . ucfirst($type) . 'Node';
    unset($t['children']);
